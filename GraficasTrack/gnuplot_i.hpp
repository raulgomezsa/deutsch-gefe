...
class Gnuplot
{
    ...
    Gnuplot& savepng(const std::string &filename  = "gnuplot_output", const int width = 0, const int heigth = 0);
    ...
    template<typename X, typename Y, typename X2, typename Y2, typename C>
    Gnuplot& plot_xyvector(const X& x, const Y& y, const X2& x2, const Y2& y2, const C& c, const std::string &title = "");
    
    template<typename X, typename Y, typename C>
    Gnuplot& plot_xyradial(const X& x, const Y& y, const C& c, const std::string &title);

	Gnuplot& set_radial(	const unsigned int x_size = 1,
                              	const unsigned int y_size = 1,
                              	const unsigned int x_max = 1,
                              	const unsigned int y_max = 1);
    ...
    ///plot x, y, x2, y2 pairs with vectors
    Gnuplot& plotfile_xyvector(const std::string &filename,
                              const unsigned int column_x = 1,
                              const unsigned int column_y = 2,
                              const unsigned int column_x2 = 3,
                              const unsigned int column_y2 = 4,
                              const unsigned int color = 1,
                              const std::string &title = "");
    
    ///plot x, y pair in radial graph
    Gnuplot& plotfile_xyradial(const std::string &filename,
                             const unsigned int column_x = 1,
                             const unsigned int column_y = 2,
                             const unsigned int color = 1,
                             const std::string &title = "");
                             
    //Imprime el fondo de la grafica
    Gnuplot& set_background(const std::string &filename,
				const std::string &filetype = "png",
                              	const unsigned int width = 500,
                              	const unsigned int heigth = 500);
...
//------------------------------------------------------------------------------
//
/// Crea una grafica a traves de vectores para pintar vector
//

template<typename X, typename Y, typename X2, typename Y2, typename C>
Gnuplot& Gnuplot::plot_xyvector(const X& x, const Y& y, const X2& x2, const Y2& y2, const C& c, const std::string &title)
{
    ...
    if (x.size() != y.size() && x.size() != x2.size() && x2.size() != y2.size())
    {
        throw GnuplotException("Length of the std::vectors differs");
        return *this;
    }
    ...
    //
    for (unsigned int i = 0; i < x.size(); i++)
        tmp << x[i] << " " << y[i] << " " << x2[i] << " " << y2[i] << std::endl;

    tmp.flush();
    tmp.close();


    plotfile_xyvector(name, 1, 2, 3, 4, c, title);
...
Gnuplot& Gnuplot::set_style(const std::string &stylestr)
{
    if (stylestr.find("lines")          == std::string::npos  &&
        ...
        stylestr.find("vectors")        == std::string::npos)   //only for one data column
...
{
    cmd("set terminal postscript eps enhanced color font 'Helvetica,10'");
...
Gnuplot& Gnuplot::savepng(const std::string &filename, const int width, const int heigth)
{
    std::ostringstream cmdstr;
    if(width == 0 || heigth == 0){
    	cmdstr << "set terminal png enhanced giant font 'Helvetica,10'"<< std::endl;
    }
    else{
    	int fontsize = (width / 512) * 9;
    	int linewidth = (width / 512) ;
    	if(fontsize < 5)
    		fontsize = 5;
    	if(linewidth < 1)
    		linewidth = 1;
    	cmdstr << "set terminal png enhanced giant size " << width << "," << heigth << " font 'Helvetica," << fontsize << "' lw "<< linewidth << std::endl;
    }
...
Gnuplot& Gnuplot::set_background(const std::string &filename,
                              	...
                              	const unsigned int heigth)
{
	...
	std::ostringstream cmdstr;
	int h = heigth*0.97;
	cmdstr << "set xrange [0:" << width-1 << "]" << std::endl << "set yrange [0:" << heigth-1 << "]" << std::endl;
	cmdstr << "set terminal wxt size " << width << "," << h << std::endl;
	...
	cmdstr << " '" << file << "' binary filetype=" << filetype << " w rgbimage";
...
//------------------------------------------------------------------------------
//
// Crea vectores a partir de un archivo
//
Gnuplot& Gnuplot::plotfile_xyvector(const std::string &filename,
                              const unsigned int column_x,
                              const unsigned int column_y,
                              ...
                              const std::string &title)
{
    ...
    cmdstr << "\"" << filename << "\" using " << column_x << ":" << column_y << ":" << column_x2 << ":" << column_y2;
    ...
    if (title == "")
        ...
        cmdstr << "with vectors head size 10,15 filled lt 1 lc "<< color;
    //
    // Do the actual plot
    //
    cmd(cmdstr.str());
...
Gnuplot& Gnuplot::set_radial(	const unsigned int x_size,
                              	...
                              	const unsigned int y_max)
{
	...
	// command to be sent to gnuplot
	//
	cmdstr << "unset border" << std::endl
	<< "set polar" << std::endl
	<< "set angles degrees" << std::endl

	<< "set grid polar " << to_string(grados) << std::endl
	<< "set grid lt 1 lc 0 lw 0.6" << std::endl

	//<< "set xrange [-" << to_string(y_max+(y_max*0.1)) << ":" << to_string(y_max+(y_max*0.1)) << "]" << std::endl
	//<< "set yrange [-" << to_string(y_max+(y_max*0.1)) << ":" << to_string(y_max+(y_max*0.1)) << "]" << std::endl
	<< "set xrange [-" << 1.1 << ":" << 1.1 << "]" << std::endl
	<< "set yrange [-" << 1.1 << ":" << 1.1 << "]" << std::endl

	<< "set xtics axis" << std::endl
	<< "set ytics axis" << std::endl
	...
	<< "set key lmargin" << std::endl

	<< "set_label(x, text) = sprintf(\"set label '%s' at (" << 1.1 << "*cos(%f)), (" << 1.1 << "*sin(%f)) center\", text, x, x)" << std::endl;
	
	for(int i = 0, j = 1; i<=360-grados; i=i+grados, ++j)
	{
		cmdstr << "eval set_label(" << i << ", \"Alumno " << j << "\")" << std::endl;
	}
...
template<typename X, typename Y, typename C>
Gnuplot& Gnuplot::plot_xyradial(const X& x, const Y& y, const C& c, const std::string &title)
{
    if (x.size() == 0 || y.size() == 0 )
    {
        throw GnuplotException("std::vectors too small");
    ...
    tmp << x[0] << " " << y[0] << std::endl;

    tmp.flush();
    tmp.close();
    
    plotfile_xyradial(name, 1, 2, c, title);

    return *this;
}  
    
Gnuplot& Gnuplot::plotfile_xyradial(const std::string &filename,
                              const unsigned int column_x,
                              const unsigned int column_y,
                              const unsigned int color,
                              const std::string &title)
{
	...
	cmdstr << "with linespoints pt 2 ps 10 lt 1 lw 1.7 lc "<< color;
